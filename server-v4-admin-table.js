...
app.get('/admin', async (req, res) => {
  ...
  const tableRows = rows.map(u => `<tr>
    ...
    <td>${u.email}</td>
    <td>${u.is_verified ? '✅' : '❌'}</td>
    <td>${new Date(u.created_at).toLocaleString('sw-TZ')}</td>
  </tr>`).join('');
  ...
  <style>
    ...
    h2{margin:0 0 12px}
    table{width:100%;border-collapse:collapse;background:#152624;border-radius:8px;overflow:hidden}
    th,td{padding:10px 8px;border-bottom:1px solid #2a423c;text-align:left;font-size:13px}
    th{background:#1d332f;color:#d9a441}
    tr:hover{background:#1d332f}
    .count{color:#8fa89f;margin-bottom:16px}
  </style></head><body>
  <h2>◆ Watumiaji wa Mbata Agent</h2>
  <p class="count">Jumla: ${rows.length}</p>
  <table><tr><th>ID</th><th>Jina</th><th>Email</th><th>Verified</th><th>Alijisajili</th></tr>${tableRows}</table>
  </body></html>`);
});
