...
const ADVISOR_TIPS = {
  finance: [
    'Weka kando angalau 10% ya kipato chako kabla ya kutumia chochote kingine.',
    'Fuatilia matumizi yako kila wiki ili kuona wapi fedha nyingi zinaenda.',
    'Weka lengo dogo la akiba kwanza (mfano wiki 4) kabla ya lengo kubwa.',
    'Epuka mikopo isiyo ya lazima - inapunguza uwezo wako wa kuweka akiba.'
  ],
  business: [
    'Angalia bidhaa zenye faida kubwa zaidi na uziuze kwa nguvu zaidi.',
    'Fuatilia stock mara kwa mara ili usikose bidhaa maarufu.',
    'Rekodi gharama zote hata ndogo - zinaathiri faida halisi.',
    'Jenga uhusiano mzuri na wateja wa mara kwa mara kwa punguzo dogo.'
  ],
  boss: [
    'Hakikisha unapokea malipo kwa wakati ili kuepuka migogoro.',
    'Weka masharti wazi ya malipo kabla ya kuanza ushirikiano.',
    'Fuatilia madereva/wapangaji wanaochelewesha malipo mara kwa mara.'
  ],
  education: [
    'Angalia matokeo ya mwanafunzi mara kwa mara, si tu mwisho wa muhula.',
    'Muhimize mwanafunzi kujadili maeneo anayopata ugumu mapema.',
    'Uwepo wa mara kwa mara darasani huathiri sana matokeo ya mtihani.'
  ],
  exam: [
    'Chagua maswali ya aina mchanganyiko (MCQ, Kweli/Sikweli, Insha) kwa tathmini bora.',
    'Weka muda wa kutosha - haraka mno huongeza makosa yasiyo ya lazima.',
    'Kagua matokeo baada ya kila mtihani kuona maeneo yanayohitaji msisitizo.'
  ]
};
...
// ===================== PROFILES =====================
app.get('/api/advisor/:module', requireAuth, (req, res) => {
  const tips = ADVISOR_TIPS[req.params.module];
  if (!tips) return res.json({ tip: null });
...
app.get('/api/finance', requireAuth, requireProfile('finance'), async (req, res) => {
  ...
  res.json({ balance: income - expense, income, expense, transactions: tx.rows, goals: goals.rows });
});
...
app.post('/api/exams/:id/questions', requireAuth, requireProfile('examiner'), async (req, res) => {
  ...
  if (!['mcq', 'true_false', 'essay'].includes(type)) return res.status(400).json({ error: 'Aina ya swali si sahihi.' });
  if (!questionText || (type !== 'essay' && !correctAnswer)) return res.status(400).json({ error: 'Jaza swali na jibu sahihi.' });
  ...
  await pool.query(
    ...
    [examId, type, questionText, JSON.stringify(type === 'true_false' ? ['Kweli', 'Sikweli'] : (options || [])), type === 'essay' ? null : correctAnswer, marks || 1, posRes.rows[0].p]
  );
  res.json({ ok: true });
});
...
app.get('/api/exams/:id/questions', requireAuth, requireProfile('student'), async (req, res) => {
  ...
  const answers = await pool.query('SELECT question_id, answer, answer_file_name FROM exam_answers WHERE attempt_id=$1', [attempt.rows[0].id]);
...
app.post('/api/exams/:id/answer-file', requireAuth, requireProfile('student'), upload.single('file'), async (req, res) => {
  const examId = req.params.id;
  const { questionId } = req.body;
  if (!req.file) return res.status(400).json({ error: 'Hakuna faili.' });
  const attempt = await pool.query('SELECT * FROM exam_attempts WHERE exam_id=$1 AND student_id=$2', [examId, req.user.id]);
  if (!attempt.rows[0] || attempt.rows[0].submitted_at) return res.status(403).json({ error: 'Huwezi kupakia sasa.' });
  const base64 = req.file.buffer.toString('base64');
  await pool.query(
    `INSERT INTO exam_answers (attempt_id, question_id, answer_file_name, answer_file_mimetype, answer_file_data) VALUES ($1,$2,$3,$4,$5)
     ON CONFLICT (attempt_id, question_id) DO UPDATE SET answer_file_name=$3, answer_file_mimetype=$4, answer_file_data=$5`,
    [attempt.rows[0].id, questionId, req.file.originalname, req.file.mimetype, base64]
  );
  res.json({ ok: true, fileName: req.file.originalname });
});

app.get('/api/exams/:id/attempts/:attemptId/answers', requireAuth, requireProfile('examiner'), async (req, res) => {
  ...
  const qs = await pool.query('SELECT * FROM exam_questions WHERE exam_id=$1 ORDER BY position', [req.params.id]);
  const ans = await pool.query('SELECT question_id, answer, marks_awarded, answer_file_name, is_correct FROM exam_answers WHERE attempt_id=$1', [att.id]);
  const ansMap = {}; ans.rows.forEach(a => ansMap[a.question_id] = a);
  const combined = qs.rows.map(q => ({ question: q, answer: ansMap[q.id] || null }));
  res.json({ attempt: att, items: combined });
});

app.get('/api/exams/:id/attempts/:attemptId/answers/:questionId/file', requireAuth, requireProfile('examiner'), async (req, res) => {
  const att = await getOwnAttempt(req.params.id, req.params.attemptId, req.user.id);
  if (!att) return res.status(404).send('Haipo.');
  const r = await pool.query('SELECT * FROM exam_answers WHERE attempt_id=$1 AND question_id=$2', [att.id, req.params.questionId]);
  const row = r.rows[0];
  if (!row || !row.answer_file_data) return res.status(404).send('Hakuna faili.');
  const buf = Buffer.from(row.answer_file_data, 'base64');
  res.setHeader('Content-Type', row.answer_file_mimetype || 'application/octet-stream');
  res.setHeader('Content-Disposition', `inline; filename="${row.answer_file_name || 'jibu'}"`);
  res.send(buf);
});

app.post('/api/exams/:id/attempts/:attemptId/grade', requireAuth, requireProfile('examiner'), async (req, res) => {
  const att = await getOwnAttempt(req.params.id, req.params.attemptId, req.user.id);
  if (!att) return res.status(404).json({ error: 'Attempt haipo.' });
  const { questionId, marksAwarded } = req.body;
  await pool.query(
    `INSERT INTO exam_answers (attempt_id, question_id, marks_awarded, graded_at) VALUES ($1,$2,$3,now())
     ON CONFLICT (attempt_id, question_id) DO UPDATE SET marks_awarded=$3, graded_at=now()`,
    [att.id, questionId, marksAwarded]
  );
  await recomputeAttemptScore(att.id);
  await logActivity(req.user.id, 'exam_grade', `Amesahihisha swali #${questionId} kwa attempt #${att.id}: ${marksAwarded} alama`);
  res.json({ ok: true });
});
...
async function gradeAttempt(examId, attemptId) {
  ...
  const ansMap = {}; ansRows.rows.forEach(a => ansMap[a.question_id] = a);
  let score = 0, total = 0, hasPending = false;
  for (const q of qs.rows) {
    total += Number(q.marks);
    if (q.type === 'essay') {
      hasPending = true;
      const existing = ansMap[q.id];
      // Insha: hatuiwezi auto-grade, tunahifadhi tu jibu na kusubiri mwalimu asahihishe
      await pool.query(
        `INSERT INTO exam_answers (attempt_id, question_id, answer) VALUES ($1,$2,$3)
         ON CONFLICT (attempt_id, question_id) DO UPDATE SET answer=COALESCE(exam_answers.answer, $3)`,
        [attemptId, q.id, existing ? existing.answer : null]
      );
      if (existing && existing.marks_awarded != null) score += Number(existing.marks_awarded);
      continue;
    }
    const given = (ansMap[q.id]?.answer || '').trim().toLowerCase();
    ...
    await pool.query(
      ...
      [attemptId, q.id, ansMap[q.id]?.answer || null, isCorrect, isCorrect ? q.marks : 0]
    );
  }
  await pool.query('UPDATE exam_attempts SET submitted_at=now(), score=$1, total_marks=$2 WHERE id=$3', [score, total, attemptId]);
  return { score, total, pendingGrading: hasPending };
}

async function recomputeAttemptScore(attemptId) {
  const r = await pool.query('SELECT COALESCE(SUM(marks_awarded),0) AS s FROM exam_answers WHERE attempt_id=$1', [attemptId]);
  await pool.query('UPDATE exam_attempts SET score=$1 WHERE id=$2', [r.rows[0].s, attemptId]);
}
