...
app.post('/api/exams/:id/submit', requireAuth, requireProfile('student'), async (req, res) => {
  ...
  const ansRows = await pool.query('SELECT * FROM exam_answers WHERE attempt_id=$1', [att.id]);
  ...
  for (const q of qs.rows) {
    ...
    await pool.query(
      ...
      [att.id, q.id, ansMap[q.id] || null, isCorrect, isCorrect ? q.marks : 0]
    );
  }
  await pool.query('UPDATE exam_attempts SET submitted_at=now(), score=$1, total_marks=$2 WHERE id=$3', [score, total, att.id]);
